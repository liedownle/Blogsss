...
2. "看不到正文"是**看漏了**：F12 页面中，小说正文展示在一行里，没往后拉着看，所以没看到。
...
第一次点击某一章获取正文时，F12 会捕捉到三个关键请求：

| entry | 请求 | 响应 | 内容说明 |
|---|---|---|---|
| 0 | `GET /read/617556/p796.html` | 200 text/html，body 仅 **422 bytes** | 无正文，返回了一段 JS，其中包含 token 及后续 token 在请求中要放的位置 |
| 1 | `GET /read/617556/p796.html?challenge=<base64>`（请求路径中带上上一个响应中下发的 token） | **302**，body 0 | 带挑战参数的跳转，**响应头下发关键放行标记** |
| 2 | `GET /read/617556/p796.html` | 200 text/html，body 明文 | **真正的正文页**，正文明文 UTF-8 内嵌 |

可以看到，第一个请求获取正文时，服务器没有直接返回正文，而是先进行了一个验证流程，然后再进行一次请求获取正文，所以正文是放在第二个请求中返回的。
在这之后的几分钟内，再次尝试获取其他章节正文时，则不会再进行验证流程，服务器会直接返回正文。
...
可以看到，第一个请求返回的 token 按照第一个请求中给出的使用方式，被加在了第二个请求的请求路径中；第三个请求则直接自动重定向成功至正文。

其余 entry 均为 CSS/JS/广告/统计请求（zepto、gtag、pubadx、a-ads 等），与正文无关。

**3. 三个请求的 cookie 和 set-cookie 值都相同**
...
- **`expires=Fri, 01-Jan-1971`（1971 年，Unix 时间戳约 1）+ `Max-Age=0`**：这两个组合在一起是标准的**"删除 cookie"指令**。`Max-Age=0` 表示"立即过期"，配合 1971 年的过期时间，作用是让浏览器把这个 cookie 删掉（如果存在的话）。
...
| 观察项 | 事实 | 含义 |
|---|---|---|
| 三个请求的 cookie | 完全一样（PHPSESSID 等不变） | 浏览器端身份没变 |
| 三个请求的 set-cookie | 完全一样，且是删除指令 | 没有新增任何凭证 |
...
- [ ] **爬取思路**：直接获取小说正文；如果未获取到，说明进入了验证环节，服务器返回的是验证 token，此时重新发起"路径中带 token"的请求通过验证，服务器会重定向到正文，然后继续爬取即可。
